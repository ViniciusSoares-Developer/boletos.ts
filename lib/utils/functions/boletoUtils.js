...
var Datas = (function () {
  ...
  Datas.prototype.comDocumento = function (_documento, locate = "usa") {
    _documento = new Date(
      moment(new Date(_documento)).add(86400000).format("YYYY-MM-DD")
    );

    if (!validarData(_documento)) {
      throw new Error(
        "O ano do documento deve ser maior que 1997 e menor que 2029"
      );
    }
  ...
  Datas.prototype.comProcessamento = function (_processamento, locate = "usa") {
    _processamento = new Date(
      moment(new Date(_processamento)).add(86400000).format("YYYY-MM-DD")
    );
  ...
  Datas.prototype.comVencimento = function (_vencimento, locate = "usa") {
    _vencimento = new Date(
      moment(new Date(_vencimento)).add(86400000).format("YYYY-MM-DD")
    );
...
var Boleto = (function () {
  var DATA_BASE = new Date(2022, 5, 29);
  ...
  Boleto.prototype.getFatorVencimento = function () {
    ...
    let diferencaEmDias = (vencimento - DATA_BASE) / (1000 * 60 * 60 * 24);

    if (diferencaEmDias > 9999) {
      throw new Error("O vencimento não pode ser maior que 9999 dias");
    }

    return Math.floor(diferencaEmDias).toString().padStart(4, "0");
  };
